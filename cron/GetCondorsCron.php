...
if ($http_response_header[0] !== "HTTP/1.1 200 OK") exit();
...
foreach ($data->channel->item as $d) {
    ...
    if ($exists < 1) {
      $stmtInsert = $mysqli->prepare("INSERT INTO torrents (et_id, torrent_hash, title, date) VALUES (?, ?, ?, ?)");
      $stmtInsert->bind_param("isss", $d->et_id, $d->info_hash, $d->title, date('Y-m-d H:i:s', strtotime($d->pubDate)));
