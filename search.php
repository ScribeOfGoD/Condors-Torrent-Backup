...
<div align="center">
    ...
    if ($search) {
        if (preg_match('/[a-f0-9]{40}/i', $search)) {
            $statement = $mysqli->prepare('SELECT * FROM torrents WHERE torrent_hash=?');
            $statement->bind_param('s', $search);
        } else {
            $conditions = advSearch($search);
            $statement = $mysqli->prepare('SELECT * FROM torrents WHERE ' . $conditions . ' ORDER BY date DESC');
        }
        $statement->execute();
